        ...
        <nav className="bg-white rounded-lg">
            <div className="flex justify-start items-center py-2 w-full">
                <Link to="/" style={{ textDecoration: 'none' }}>
                    <div className="flex items-center gap-3 px-3">
                        <img className="w-20 h-10" src={logo} alt="Logo" />
                        <span className="text-lg font-medium text-gray-800" style={{ fontSize: '25px' }}>
                            BrainBuffs
                        </span>
                    </div>
                </Link>
                <button onClick={toggleMenu} className="lg:hidden ml-auto" style={{ backgroundColor: 'white' }}>
                    {/* Hamburger icon */}
                </button>
                <div className={`${isMenuOpen ? 'hidden' : 'lg:flex'} items-center justify-between px-10 flex-grow w-full`}>
                    <div className="flex items-center gap-[2px]">
                        {menuLinks.map(({ to, label }, index) => (
                            <Link key={index} to={to} style={{ textDecoration: 'none' }}>
                                <button
                                    className="cursor-pointer [border:none] py-2.5 px-4 bg-[transparent] rounded-lg flex flex-row items-center justify-start gap-[12px]"
                                >
                                    <div className="relative text-sm leading-[20px] font-poppins text-light-theme-subheading-text text-left">
                                        {label}
                                    </div>
                    ...
                    <div className="relative">
                        <button className="cursor-pointer border-none py-2.5 px-3 bg-transparent rounded-lg flex items-center gap-2" onClick={toggleDropdown}>
                            {/* Dropdown arrow */}
                            <div className="text-sm font-poppins text-light-theme-subheading-text">
                                {isLoggedIn ? `Hello, ${user.email}` : 'Hello, Sign in'}
                            </div>
                            ...
                            <div className="absolute top-full left-0 mt-2 bg-white shadow-md rounded-lg">
                                <ul className="list-none p-0">
                                    {isLoggedIn ? (
                                        <>
                                            <li>
                                                <Link to="/manageAccount" style={{ textDecoration: 'none', display: 'block', padding: '8px 16px' }} onClick={toggleDropdown}>
                                                    Manage Account
                                                </Link>
                                            </li>
                                            <li>
                                                <button onClick={handleLogout} style={{ border: 'none', background: 'none', display: 'block', padding: '8px 16px' }}>
                                                    Sign Out
                                                </button>
                                            </li>
                                        </>
                                        ...
                                        <>
                                            <li>
                                                <Link to="/signin" style={{ textDecoration: 'none', display: 'block', padding: '8px 16px' }} onClick={toggleDropdown}>
                                                    Sign In
                                                </Link>
                                            </li>
                                            <li>
                                                <Link to="/signup" style={{ textDecoration: 'none', display: 'block', padding: '8px 16px' }} onClick={toggleDropdown}>
                                                    Sign Up
                                                </Link>
                                            </li>
                                        </>
